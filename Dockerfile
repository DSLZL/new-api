# syntax=docker/dockerfile:1.7

FROM oven/bun:1@sha256:0733e50325078969732ebe3b15ce4c4be5082f18c4ac1a0f0ca4839c2e4e42a7 AS builder-default

WORKDIR /build
COPY web/default/package.json web/default/bun.lock ./
RUN --mount=type=cache,target=/root/.bun/install/cache,id=bun-cache,sharing=locked \
    bun install --frozen-lockfile
COPY ./web/default ./
COPY ./VERSION ./
RUN --mount=type=cache,target=/root/.bun/install/cache,id=bun-cache,sharing=locked \
    DISABLE_ESLINT_PLUGIN='true' VITE_REACT_APP_VERSION=$(cat VERSION) bun run build

FROM oven/bun:1@sha256:0733e50325078969732ebe3b15ce4c4be5082f18c4ac1a0f0ca4839c2e4e42a7 AS builder-classic

WORKDIR /build
COPY web/classic/package.json web/classic/bun.lock ./
RUN --mount=type=cache,target=/root/.bun/install/cache,id=bun-cache,sharing=locked \
    bun install --frozen-lockfile
COPY ./web/classic ./
COPY ./VERSION ./
RUN --mount=type=cache,target=/root/.bun/install/cache,id=bun-cache,sharing=locked \
    VITE_REACT_APP_VERSION=$(cat VERSION) bun run build

FROM golang:1.26.1-alpine@sha256:2389ebfa5b7f43eeafbd6be0c3700cc46690ef842ad962f6c5bd6be49ed82039 AS builder2
ENV GO111MODULE=on CGO_ENABLED=0

ARG TARGETOS
ARG TARGETARCH
ENV GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH:-amd64}
ENV GOEXPERIMENT=greenteagc

WORKDIR /build

COPY go.mod go.sum ./
RUN --mount=type=cache,target=/go/pkg/mod,id=go-mod-cache,sharing=locked \
    go mod download

COPY . .
COPY --from=builder-default /build/dist ./web/default/dist
COPY --from=builder-classic /build/dist ./web/classic/dist
RUN --mount=type=cache,target=/go/pkg/mod,id=go-mod-cache,sharing=locked \
    --mount=type=cache,target=/root/.cache/go-build,id=go-build-cache,sharing=locked \
    go build -ldflags "-s -w -X 'github.com/QuantumNous/new-api/common.Version=$(cat VERSION)'" -o new-api

FROM debian:bookworm-slim@sha256:f06537653ac770703bc45b4b113475bd402f451e85223f0f2837acbf89ab020a

RUN --mount=type=cache,target=/var/cache/apt,id=apt-cache,sharing=locked \
    --mount=type=cache,target=/var/lib/apt,id=apt-lib,sharing=locked \
    apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates tzdata libasan8 wget \
    && update-ca-certificates

COPY --from=builder2 /build/new-api /new-api
EXPOSE 3000
WORKDIR /data
ENTRYPOINT ["/new-api"]
