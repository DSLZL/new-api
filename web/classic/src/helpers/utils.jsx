...
export const formatDynamicPriceSummary = (
  ...
  const expr = typeof billingExpr === 'string' ? billingExpr.trim() : '';
  const ratioText =
    usedGroupRatio !== undefined && usedGroupRatio !== null
      ? ` ${t('分组倍率')} ${usedGroupRatio}`
      : '';
  if (!expr) {
    return `${t('动态计费已启用')}${ratioText}`;
  }
  const compact = expr.length > 72 ? `${expr.slice(0, 72)}...` : expr;
  return `${t('动态计费')}: ${compact}${ratioText}`;
};

// 兼容旧版 DynamicPricingBreakdown：从表达式中抽取 tier(...)
export const parseTiersFromExpr = (expr) => {
  if (!expr || typeof expr !== 'string') return [];
  const text = expr.replace(/\s+/g, ' ');
  const regex = /tier\s*\(([^)]*)\)/gi;
  const tiers = [];
  let m;
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    for (const seg of raw.split(',')) {
      ...
      switch (k) {
        case 'input':
        case 'p':
          fields.input_price = Number.isFinite(num) ? num : fields.input_price;
          break;
        case 'output':
        case 'c':
          fields.output_price = Number.isFinite(num)
            ? num
            : fields.output_price;
          break;
        case 'cache':
          fields.cache_price = Number.isFinite(num) ? num : fields.cache_price;
          break;
        case 'cache_create':
          fields.cache_create_price = Number.isFinite(num)
            ? num
            : fields.cache_create_price;
          break;
        case 'image':
          fields.image_price = Number.isFinite(num) ? num : fields.image_price;
          break;
        case 'audio_input':
          fields.audio_input_price = Number.isFinite(num)
            ? num
            : fields.audio_input_price;
          break;
        case 'audio_output':
          fields.audio_output_price = Number.isFinite(num)
            ? num
            : fields.audio_output_price;
          break;
        case 'label':
          fields.label = v || '';
          break;
